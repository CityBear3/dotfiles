...
Each Task loop starts with one root-granted baseline leaf. Implementation, verification, findings integration, triage, and correction use that leaf serially. After a fresh verifier `PASS`, a Task loop with at least two policy-selected independent source reviewers may request temporary expansion; only the root grants up to three total Task leaves or the smaller available capacity. Only those source reviewers use the expansion, and it is revoked before findings integration, triage, or correction. Free runtime capacity does not grant permission, and unavailable expansion queues reviewers in policy order.

The Task-loop owner builds one in-memory current-head Verification Matrix after the Task commit. Each observable obligation maps to a bounded command or check, expected observation, and `FAIL` or `BLOCKED` mismatch category. A changed head, range, controlling authority, or material verification route invalidates the matrix. The managed `implementation-verifier` remains check-only in its bounded `workspace-write` sandbox, uses `gpt-5.6-sol` with `medium` reasoning effort, executes the matrix in mechanical fail-fast order, and leaves semantic judgment to policy-selected reviewers. Clean planned verification uses the minimal required target/range/source checks; dirty standalone snapshots retain a fuller staged, unstaged, untracked, and bounded-file fingerprint.

When correction changes reviewed head `H1` to `H2`, one bounded correction commit is followed by a rebuilt matrix and fresh `H2` verification. The same complete reviewer set then inspects `H1..H2` first but returns fresh verdicts for the full `base..H2` target. Prior reports guide traversal only; they never authorize the new head. Ordinary full traversal is retained when the delta escapes authorization, changes a material contract or shared interface, changes base or policy, lacks complete prior evidence, exposes another finding, or cannot prove prior inspected areas unaffected.

The implementer may batch independent initial authority reads, repository searches, relevant file reads, and Git inspection only when each underlying result stays attributable and no result changes the next action. The ordered behavior-change stages remain `focused RED -> production edit -> focused GREEN -> refactor while green`; independent mechanical post-edit checks may be grouped only after GREEN. Reports preserve the actual pre-production RED and its reason. A historical RED gap is disclosed but is evaluated separately from current-head defects, current evidence, contract deviation, and fresh Acceptance gates.
